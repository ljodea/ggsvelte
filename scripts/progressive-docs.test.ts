...
describe("progressive Docs journey", () => {
  ...
  it("derives getting-started headings from guide markdown, not the lesson fold", () => {
    ...
    const headings = route?.headings?.map(({ id, title }) => ({ id, title }));
    expect(headings).toEqual(
      expect.arrayContaining([
        { id: "install", title: "Install" },
        { id: "a-complete-svelte-file", title: "A complete Svelte file" },
        { id: "the-portablespec-contract", title: "The PortableSpec contract" },
      ]),
    );
    // Progressive walkthrough step titles must not leak into the guide page.
    for (const step of SAKURA_STEPS) {
      expect(headings?.some((h) => h.id === step.id)).toBe(false);
    }
  });
