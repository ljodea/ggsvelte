...
async function generatedSource(
  ...
  return await formatGeneratedSource(DEFAULT_OUTPUT, raw);
}

/** Default-path protocol handle (tests may still target custom output paths). */
export const playgroundSeedsArtifact = defineArtifact({
  path: DEFAULT_OUTPUT,
  label: "apps/docs/src/lib/generated/playground-seeds.ts",
  regenerateWith: "playground:seeds:gen",
  dependsOn: [manifestArtifact],
  build: () => generatedSource(EXAMPLES, SOURCE_SAMPLES, defaultLoadSpec),
});
...
export async function generatePlaygroundSeeds(
  ...
  // Custom paths (unit tests) keep the previous direct path; default path uses protocol.
  if (
    output === DEFAULT_OUTPUT &&
    options.entries === undefined &&
    options.samples === undefined &&
    options.loadSpec === undefined
  ) {
    if (options.check === true) {
      await playgroundSeedsArtifact.check();
      return;
    }
    await playgroundSeedsArtifact.write();
    return;
  }
...
if (import.meta.main || import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  await playgroundSeedsArtifact.cli();
}
