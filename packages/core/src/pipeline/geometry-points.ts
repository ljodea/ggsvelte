...
export function pointsBatch(
  ...
  // point / count / dotplot / geom_sf point family share this builder.
  // SfParams has size/alpha (not shape); DotplotParams adds binwidth/dotsize.
  const geom = binding.layer.geom;
  const params =
    geom === "point" || geom === "count" || geom === "dotplot" || geom === "sf"
      ...
      : {};
  const paramShape =
    geom === "point" || geom === "count" || geom === "dotplot" ? params.shape : undefined;
