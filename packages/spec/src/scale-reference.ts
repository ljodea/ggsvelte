/**
 * SCALE_REFERENCE — per-Scale* component API docs derived from
 * SCALE_CAPABILITIES (helpers, families, aesthetics) and SpecDeclarations
 * (param descriptions for position / color / style scale schemas).
 *
 * Used by `/reference/scales`. Shells under packages/svelte/src/lib/scale/
 * are declaration-only; document this catalog instead of TypeDoc.
 */
import {
  SCALE_CAPABILITIES,
...
function buildScaleReference(): Readonly<Record<string, ScaleReferenceEntry>> {
 ...
 * (e.g. color_continuous, x_log10).
 */
export const SCALE_REFERENCE: Readonly<Record<string, ScaleReferenceEntry>> = buildScaleReference();
