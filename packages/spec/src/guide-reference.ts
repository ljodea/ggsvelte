/**
 * GUIDE_REFERENCE — per-guide API docs derived from SpecDeclarations guide
 * schemas (*GuideSpec) and aesthetic compatibility rules.
 *
 * Guides are declaration-only Svelte shells keyed by aesthetic channel:
 * `<GuideLegend channel="color" position="bottom"/>` assembles
 * `guides: { color: { type: "legend", position: "bottom" } }`. Used by
 * `/reference/guides`.
 */
...
function buildGuideReference(): Readonly<Record<GuideTypeName, GuideReferenceEntry>> {
 ...
 * Params come from *GuideSpec; channels mirror guideStructuralErrors.
 */
