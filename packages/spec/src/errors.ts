/**
 * Agent-facing error contract (plan: "Validation for agents") + the error
 * catalog. Every validation diagnostic is a SpecError:
 *
 *   { code, path, message, allowed?, fix? }
 *
 * - `code` is a stable, documented identifier from ERROR_CODES.
 * - `path` is a JSON pointer into the offending spec ("" = root).
 * - `message` is one plain-English sentence, snapshot-tested.
 * - `allowed` lists legal values where the set is small and closed.
 * - `fix` is a machine-actionable suggestion: what to change, with an example.
 *
