...
describe("scale-child parity (all 80 shells)", () => {
  it(`enumerates exactly 80 helpers (got ${String(helpers.length)})`, () => {
    expect(helpers).toHaveLength(80);
  });
...
/**
 * The 80-shell sweep above runs on ScaleRegistryHost, which provides the
