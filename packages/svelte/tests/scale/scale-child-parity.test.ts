...
describe("scale-child parity (all 66 shells)", () => {
  it(`enumerates exactly 66 helpers (got ${String(helpers.length)})`, () => {
    expect(helpers).toHaveLength(66);
  });
...
/**
 * The 66-shell sweep above runs on ScaleRegistryHost, which provides the
