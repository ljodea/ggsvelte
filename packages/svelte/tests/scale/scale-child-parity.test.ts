...
describe("scale-child parity (all 92 shells)", () => {
  it(`enumerates exactly 92 helpers (got ${String(helpers.length)})`, () => {
    expect(helpers).toHaveLength(92);
  });
...
/**
 * The 92-shell sweep above runs on ScaleRegistryHost, which provides the
