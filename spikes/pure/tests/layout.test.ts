...
import { MetricsTableMeasurer } from '../src/measure.js';
import { FONT_METRICS } from '../src/font-metrics.js';
  ...
  it('a hypothetical third pass almost never moves margins, and never far', () => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const yWidest = Math.max(
        0,
        ...r.y.ticks.filter((t) => t.labeled).map((t) => measurer.measureWidth(t.label, theme.fontSize)),
      );
