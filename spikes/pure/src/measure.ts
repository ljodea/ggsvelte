/**
 * M0a-3 spike — TextMeasurer contract + the canonical deterministic measurer.
 *
 ...
 * sum. The drift is quantified in spikes/browser/tests/browser/measure-drift.test.ts
 * and absorbed by margin quantization (see layout.ts).
 */
...
export class MetricsTableMeasurer implements TextMeasurer {
  ...
  measureWidth(text: string, fontSizePx: number): number {
    ...
    for (const ch of text) {
      ...
      sum += adv === undefined ? defaultAdvance : adv;
    }
