/**
 * M0a-2 spike: value-stable discrete scale state.
 *
 * Implements the plan's "Scale stability" contract as a pure, dependency-free
 * prototype:
 ...
 *
 * Throwaway spike code: correctness of semantics matters, micro-perf does not.
 */
...
export function adoptScaleState(json: string): ScaleState {
  ...
  return {
    ...
    exhaustWarned: s.exhaustWarned === true,
  };
}
...
export function trainDiscrete(
  ...
  if (spec.domain !== undefined) {
    ...
    if (unknownKeys.size > 0) {
      const offenders = [...unknownKeys].map(decodeKey);
      warnings.push({
        code: "out-of-domain",
    ...
    return {
      mode: "pinned",
      state,
      domain: [...pinned.keys()].map(decodeKey),
  ...
  if (mode === "grow" && prevState != null) {
    if (prevState.version !== SCALE_STATE_VERSION) {
      warnings.push({
        code: "version-mismatch",
        message: `Stored scale state has schema version ${String(prevState.version)}; expected ${SCALE_STATE_VERSION}. Starting fresh.`,
      });
  ...
  const domainKeys = [...present].sort(
    (a, b) => assignments.get(a)! - assignments.get(b)!,
  );

  return {
    mode,
    state,
    domain: domainKeys.map(decodeKey),
