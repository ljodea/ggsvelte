...
**Breaking (pre-1.0):** spec-driven charts (`layers` prop, `spec`, or headless `runPipeline` / `renderToSVGString`) using specialty geoms/stats must call `registerAll()` once (or a per-family register function). Missing registration fails loudly with a "not registered in this build" error naming the fix. `ggsvelte-render` (CLI) registers the full grammar itself — no change.
